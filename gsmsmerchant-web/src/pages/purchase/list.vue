    ...
    <div class="panel panel-default">
        <div class="panel-body">
            <el-form ref="form" :model="params" :inline="true" class="form-inline pull-left add-qx-bottom">
                <div class="form-group">
                    <el-form-item>
                        <span class="demonstration">发货时间</span>
                    ...
                    <el-form-item>
                        <el-select filterable size="medium" v-model="params.need_logistics" class="input-cos" placeholder="请选择发货类型">
                            <el-option value="">所有</el-option>
                            <el-option v-for="(item, index) in needLogisticsList" :key="index" :value="item.value" :label="item.name"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item>
        ...
        <el-scrollbar style="height:100%">
            <el-table :data="dataList" border height="532px">
                ...
                <el-table-column align="center" label="卡片数量">
                    <template slot-scope="scope">
                        <el-button @click="toDetail(scope.row)" class="text-info" type="text" size="medium">{{scope.row.product_num}}</el-button>
                    </template>
                </el-table-column>
                ...
                <el-table-column align="center" label="采购单价（元）">
                    <template slot-scope="scope">
                        <span>{{ scope.row.per_price | AmountFilter}}</span>
                    </template>
                </el-table-column>
                ...
                <el-table-column align="center" label="下发门店">
                    <template slot-scope="scope">
                ...
                <el-table-column align="center" label="发货类型">
                    <template slot-scope="scope">
                        {{scope.row.need_logistics | EnumFilter('NeedLogistics')}}<br/>
                        <span v-if="scope.row.logistics_company!=''">{{scope.row.logistics_company}}: {{scope.row.logistics_order_no}}</span>
                    </template>
                </el-table-column>
                <el-table-column align="center" label="订单状态">
                    <template slot-scope="scope">
                        <div :class="orderStatusClassFilter(scope.row.order_status)">{{scope.row.order_status | EnumFilter('OrderStatus')}}</div>
                    </template>
                </el-table-column>
                <el-table-column fixed="right" align="center" label="操作">
                    <template slot-scope="scope">
                        <el-button v-if="scope.row.order_status=='0'" 
                        type="text" href="#" class="text-info" @click="exportCard(scope.row)">导出卡号</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </el-scrollbar>

        <card-modal ref='CardListShow'></card-modal>
...
import CardListShow from './card.list.vue'

export default {
  components: {
    'card-modal': CardListShow,
  },
  data() {
    return {
      heightMax: '100%',
  ...
  methods: {
    ...
    queryDataList() {
      ...
      this.$get("/product/purchase/query", this.params).then(response => {
    ...
    toDetail(item) {
      this.$refs.CardListShow.setModal(item)
    },
    search() {
      this.params.pi = 1
      this.queryDataList()
    ...
    exportCard(item) {
      window.open(
        process.env.service.url +
        "/product/purchase/export?&order_no=" + item.order_no
      );
    }
  },
  filters: {
